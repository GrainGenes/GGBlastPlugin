...
constructor: function( args ) {
    ...
    /*
    $.get(thisB.pre+"/loginstate",function(data) {
        ...
        setInterval(function(){
            $.get(thisB.pre+'/jobactive/checkchanges?get', function(changes) {
                if (changes.jobChanges.length) {
                    console.log('job changes',changes.jobChanges)

                    browser.jbconnect.queue.queueUpdate(changes.jobChanges)
                }
                if (changes.trackChanges.length) {
                    console.log('track changes',changes.trackChanges)
                    
                    for(let i in changes.trackChanges) {
                        newTrackHandler ('new',changes.trackChanges[i]);
                    }
                }
                
                //let jobs = changes.jobChanges;
            });
            let lastActive = 0; 
            $.get(thisB.pre+'/jobactive/get', function(data) {
                if (data.active !== lastActive) {
                    console.log('changes active',data.active)

                    if (data.active===0) $("div.flapEx").removeClass("cogwheel");
...
setupEventTraps: function() {
    ...
    $.get(this.pre+'/track/get', function(resData, jwres) {console.log(resData);});
