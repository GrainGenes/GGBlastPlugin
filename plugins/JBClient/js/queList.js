...
define(function(){
    return {
        ...
        initQueue(browser) {
            ...
            // x io.socket.get(this.pre+'/jobactive', function(resData, jwres) {console.log(resData);});
        ...
        queueUpdate(jobs) {
            for(let i in jobs) {
                ...
                if (this.jobs[data.id]) {
                    // job exists
                    this.jobs[data.id].state = data.state;
                    this.jobs[data.id].name = data.name;
                    $('#j-hist-grid #'+data.id+" .state").attr('questate',this.getQueState(data.state));
                    $('#j-hist-grid #'+data.id+" .name").html(data.name);
                    ...
                    $('#j-hist-grid #head').after(
                            "<tr class='j-hist-item' id='"+data.id+"'>"
                            +"<td>"+data.id+"</td>"
                            +"<td class='state' questate='"+this.getQueState(data.state)+"'></td>"
                            +"<td class='name'>"+data.name+"</td>"
                            +"</tr>");                
                    this.handleJobQueueEvent(data.id);
        ...
        doGetQueue() {
            ...
            thisb.getJobs(function(data){
                ...
                for (var x in jdata) {
                    ...
                    //if (typeof jdata[x] === 'object') {  // not sure why this check is needed...but it fixes the problem.
